...
def line_sphere_intersection_numpy(totem_pos, totem_radius, rays_d, rays_o, use_min=True):
    ...
    '''
    ###SAGE_CUSTOM Debugging
    # import pdb; pdb.set_trace()
    # # Plot out totem positions as points using scatter3d and plot out rays as lines
    # from mpl_toolkits import mplot3d
    # import matplotlib.pyplot as plt #CAVEAT move outside this function for speed
    # np.save("rays_d.npy", rays_d)
    # np.save("rays_o.npy", rays_o)
    # ax = plt.axes(projection='3d')
    # xdata = np.unique(totem_pos, axis=0)[:,0]
    # ydata = np.unique(totem_pos, axis=0)[:,1]
    # zdata = np.unique(totem_pos, axis=0)[:,2]
    # ax.scatter3D(xdata, ydata, zdata, c=zdata, cmap='Dark2');
    # for i,ray in enumerate(rays_o):
    #     x_start = ray[0]
    #     y_start = ray[1]
    #     z_start = ray[2]
    #     xs = x_start + np.arange(10)*rays_d[i][0]
    #     ys = y_start + np.arange(10) * rays_d[i][1]
    #     zs = z_start + np.arange(10) * rays_d[i][2]
    #     ax.plot3D(xs, ys, zs, 'gray')
    # # plt.savefig("debugging_rays.png")
...
def cam_rays_to_totem_rays_numpy(totem_radius, near, cam_rays_o, cam_rays_d, totem_pos, W, H, K, ior_totem, ior_air=1.0):
    ...
    '''
    cam_rays_o = cam_rays_o.cpu().detach().numpy() ###SAGE_CUSTOM fixing data type CAVEAT this might need to be fixed for joint_totem_optimization
    cam_rays_d = cam_rays_d.cpu().detach().numpy() ###SAGE_CUSTOM fixing data type CAVEAT this might need to be fixed for joint_totem_optimization
    cam_ray_o = cam_rays_o[0]
    ###SAGE_CUSTOM removing args param
    ...
    ###SAGE_CUSTOM
    totem_pos = totem_pos[valid_idx_1]
    ###
    ...
    ###SAGE_CUSTOM
    totem_pos = totem_pos[valid_idx_2]
    ###
    ...
    ###SAGE_CUSTOM
    totem_pos = totem_pos[valid_idx_3]
    ###
    return totem_rays_o, totem_rays_d, valid_idx_1, valid_idx_2, valid_idx_3


def get_totem_rays_numpy(args, data, coords, totem_idx, totem_pos, n_rays=None):
    '''
    ...
    ###SAGE_CUSTOM hardcoding swap
    # W, H, K = data['W'], data['H'], data['K']
    H = 3840
    W = 5760
    K = np.load('calib.npy', allow_pickle=True).item()['mtx']
    ...
    if n_rays:
        ...
        for chunk_idx in range(n_chunks):
            ...
            totem_rays_o_chunk, totem_rays_d_chunk, valid_idx_1, valid_idx_2, valid_idx_3 = cam_rays_to_totem_rays_numpy(
                args, cam_rays_o_chunk, cam_rays_d_chunk, totem_pos, W, H, K, ior_totem)
        ...
        assert len(
            ys_input) >= n_rays, "Totem has gone too far from its position in the mask, not enough rays to sample"
        ...
        totem_rays_o_input, totem_rays_d_input, valid_idx_1, valid_idx_2, valid_idx_3 = cam_rays_to_totem_rays_numpy(
            args, cam_rays_o_input, cam_rays_d_input, totem_pos, W, H, K, ior_totem)
...
def get_refracted_ray_torch(S1, N, n1, n2):
    ...
    '''
    return n1 / n2 * torch.cross(N, torch.cross(-N, S1)) - N * torch.sqrt(
        1 - n1 ** 2 / n2 ** 2 * torch.unsqueeze(torch.sum(torch.cross(N, S1) * torch.cross(N, S1), dim=1), dim=1))
...
def get_totem_rays_torch(args, ys_input, xs_input, device, data, totem_idx, totem_pos, n_rays=None):
    ...
    totem_rays_o_input, totem_rays_d_input = cam_rays_to_totem_rays_torch(args, cam_rays_o_input, cam_rays_d_input,
                                                                          totem_pos, W, H, K, ior_totem)
    rays_o = totem_rays_o_input
    rays_d = totem_rays_d_input
    target_rgbs = img[ys_input, xs_input]
...
class LearnTotemPos(nn.Module):
    def __init__(self, initial_totem_pos, totem_pos_residual, req_grad, device):
        ...
        self.totem_pos_residual = nn.Parameter(torch.from_numpy(totem_pos_residual.astype('float32')),
...
def bb_intersection_over_union_torch(boxA, boxB):
    ...
    interArea = torch.max(torch.tensor([0, xB - xA + 1], dtype=torch.float32)).item() * torch.max(
        torch.tensor([0, yB - yA + 1], dtype=torch.float32)).item()
