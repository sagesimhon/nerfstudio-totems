...
###SAGE_CUSTOM commenting these out
...
def create_nerf(args):
    ...
    # Load checkpoints
    ckpts = [os.path.join(base_dir, exp_name, f) for f in sorted(os.listdir(os.path.join(base_dir, exp_name))) if
             'tar' in f]
...
###SAGE_CUSTOM making it not an official parser
# def config_parser():
#     args = {}
#
...
def train(args):
    ...
    if args.optimize_totems:
        ...
        scheduler_totem_pos = MultiStepLR(opt_totem_pos, milestones=list(range(0, n_iters - n_iters_freeze_totem, 100)),
                                          gamma=args.totem_gamma)

    ## If not optimizing totems, precompute totem rays before training
    else:
        all_precomputed_rays = []
        for totem_idx in range(n_totems):
            rays_o, rays_d, ys, xs, target_rgbs = get_totem_rays_numpy(args, data, totem_idx,
                                                                       initial_totem_pos[totem_idx], n_rays=None)
    ...
    for i in trange(start, n_iters):  # Iterations 1, 2, ..., n_iters
        ...
        if args.optimize_totems:
            ...
            rays_o_np, rays_d_np, ys, xs, target_rgbs = get_totem_rays_numpy(args, data, totem_idx, totem_pos_np,
                                                                             n_rays=args.n_rand)

            # Compute again in pytorch
            rays_o, rays_d, ys, xs, target_rgbs = get_totem_rays_torch(args, ys, xs, device, data, totem_idx, totem_pos,
                                                                       n_rays=args.n_rand)
        ...
        if i % args.i_print == 0:
            if args.optimize_totems:
                iou_loss_print = np.mean(
                    [totem_pose_iou_loss(args, data, totem_idx, totem_pos).item() for totem_idx in range(n_totems)])
                tqdm.write(
                    f"[TRAIN] Iter: {i} Loss: {loss.item():0.8f} MSE:{img_loss.item():0.8f} IoU: {iou_loss_print}  PSNR: {psnr.item():0.8f}")
                for totem_idx in range(n_totems):
                    tqdm.write(f"[TRAIN] Iter: {i} Totem{totem_idx + 1}: {learned_totem_pos_numpy[totem_idx]}")
            else:
                tqdm.write(
                    f"[TRAIN] Iter: {i} Loss: {loss.item():0.8f} MSE:{img_loss.item():0.8f} PSNR: {psnr.item():0.8f}")
        ...
        if i % args.i_test == 0:
            ...
            with torch.no_grad():
                ...
                img_loss = img2mse(rgb * torch.unsqueeze(test_mask, dim=2),
                                   target_rgbs * torch.unsqueeze(test_mask, dim=2))
                psnr = mse2psnr(img_loss)
...
def test(args, downsample=4):
    ...
    if args.render_totem:
        ...
        for totem_idx in range(n_totems):
            ...
            rays_o, rays_d, ys, xs, target_rgbs = get_totem_rays_numpy(args, data, totem_idx, all_totem_pos[totem_idx],
                                                                       n_rays=None)
            rays_np = np.stack([rays_o, rays_d], axis=0)
            ...
            with torch.no_grad():
                ...
                print(
                    f'Totem view #%02d rendered in %.4f seconds: {render_path}' % (totem_idx, time.time() - start_time))
            ...
            up, down, left, right = max(0, up - margin), min(H, down + margin), max(0, left - margin), min(W,
                                                                                                           right + margin)
